...
class Model_Search extends Model_Base
{
	...
	public function ogMeta()
	{
	    ...
	    if ($currentId === $storeId)
	    {
    		...
    		if ($record)
    		{
        		...
        		echo '<meta property="og:url" content="http://' . $_SERVER['HTTP_HOST'] . filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL) . '" />';
	...
	public function prosperTitle($title)
	{
		...
		if (is_page($page))
		{
			if (get_query_var('queryParams') || $startQuery = $this->_options['Starting_Query'])
			{
				$params   = $this->getUrlParams();
